...
> 计划一至八核心功能均已完成并合入 main，288 项核心单元测试全部通过（foobar2000 v2.25，Apple Silicon）。详见 [`docs/superpowers/STATUS.md`](docs/superpowers/STATUS.md)。
...
- 五级取词管线，可配置顺序与启停。
  ...
  4. **网易云音乐** — EAPI 加密通道（AES-128-ECB + MD5），含逐字 YRC 格式解析。
...
- **逐字高亮（Karaoke / YRC）**：支持网易云 YRC 括号格式及 LRC 行内音节时标 `[mm:ss.xx]` 解析，实现音节级动态富文本渐进高亮（可设置中开启/关闭）。
...
│   ├── model/      LyricData 歌词模型（包含 Syllable 音节结构）
│   ├── net/        UrlEncode / JsonField / Base64
│   ├── parser/     LrcParser / LrcSerializer（含 YRC 逐字行与音节解析）
...
│   └── sync/       SyncEngine（offset + 音节定位 + 穿插无时标行）
...
- **已完成**：核心解析/同步引擎、嵌入式面板、桌面歌词浮窗、多数据源（LrcLib/网易云/QQ音乐）、手动搜索微调、偏好设置、播放列表 Cmd+F 模糊搜索定位、逐字高亮（Word-level Highlight / YRC）、按单词边界智能换行（Word Wrapping）。
- **后续可选拓展**：
  - 双语歌词/翻译并排显示
  - Mini 模式（单行紧凑浮动面板）
  - 第三方流媒体源扩展 / Last.fm scrobbling（非重点需求）
